...
<div class="content-header row">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0 d-none" style="display: none !important;">Scriptures
                </h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active">Scriptures</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <table class="datatables-basic table">
                    ...
                    <tbody class="row_position">
                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-outline-info" data-bs-toggle="modal" data-bs-target="#scripturedetail{{ $row['id'] }}">
                                    Details
                                </button>

                                <div class="modal fade text-start" id="scripturedetail{{ $row['id'] }}" tabindex="-1" aria-labelledby="myModalLabel18" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" id="myModalLabel18">Scripture Details</h4>
                            ...
                            <td>
                                <div class="d-flex flex-row">
                                    <a href="{{ route('scriptures.edit', $row['id']) }}" title="Edit"
                                        class="btn btn-info me-1">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <a href="javascript:void(0)" title="Delete"
                                        data-url="{{ route('scriptures.delete', $row['id']) }}"
...
<div class="modal fade text-start" id="addItemModal" tabindex="-1" aria-labelledby="myModalLabel22" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">{{ isset($editdata) ? 'Edit' : 'Add' }} Scripture</h4>
                <button type="button" class="btn-close" aria-label="Close" @if (isset($editdata))
                    onclick="window.location.href=`{{ route('scriptures') }}`" @else data-bs-dismiss="modal"
                    @endif></button>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data" id="validationform"
                        action="{{ isset($editdata) ? route('scriptures.edit', $editdata['id']) : route('scriptures.add') }}">
                        @csrf
                        @if (isset($editdata))
                        @method('PATCH')
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 col-12">
                                <div class="mb-1">
                                    <label class="form-label" for="publish_detail">Publish Detial</label>
                                    <input type="text" class="form-control" name="publish_detail" id="publish_detail"
                                        value="{{ isset($editdata) ? $editdata['publish_detail'] : '' }}"
                                        placeholder="Enter Publish Detail">
                                    <p class="text-danger">{{ $errors->first('publish_detail') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4 col-12">
                                <div class="mb-1">
                                    ...
                                    <input type="text" class="form-control" name="author" id="author"
                                        value="{{ isset($editdata) ? $editdata['author'] : '' }}"
                            ...
                            <div class="col-md-4 col-12">
                                <div class="mb-1">
                                    ...
                                    <input type="text" class="form-control" name="root_language" id="root_language"
                                        value="{{ isset($editdata) ? $editdata['root_language'] : '' }}"
                                        placeholder="Enter Root Language">
                                    <p class="text-danger">{{ $errors->first('root_language') }}</p>
                                </div>
                            </div>
                        ...
                        <div class="col-12">
                            <button type="button" class="btn btn-flat-secondary waves-effect" aria-label="Close"
                                @if(isset($editdata)) onclick="window.location.href=`{{ route('scriptures') }}`" @else
...
<script>
    ...
    $('#addItemModal').on('hidden.bs.modal', function() {
        window.location.href = "{{ route('scriptures') }}";
    })
</script>
@else
<script>
...
<script>
    ...
    if (dt_basic_table.length) {
        ...
        $('div.head-label').html('<h6 class="mb-0">Scriptures</h6>');
    }
    ...
    function changeStatus(id) {
        $.ajax({
            url: "{{ route('scriptures.change-status') }}",
            type: "POST",
            data: {
                '_token': '{{ csrf_token() }}',
                'id': id,
